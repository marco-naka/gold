...
/**
 * Cancella i dati personali mantenendo il minimo per le statistiche e per la prova
 * dell'estrazione: l'ID della giocata, lo stato, la sorgente e la data.
 * Irreversibile: è il punto in cui il concorso smette di conservare dati di persone.
 */
export const purgePersonalData = () =>
  transaction(async (entries) => {
    ...
    return { entries: purged, result: { ok: true, count: purged.length } };
  });
