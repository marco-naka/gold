...
const en = {
  meta: {
    ...
    /** Congiunzione per l'elenco degli asset: "Bitcoin (Lightning), USDt or XAUT". */
  ...
  hero: {
    ...
    subtitle: (event) =>
      `Pay with crypto at Lugano shops running a NAKA POS and enter the prize draw for Tether Gold (XAUT). During ${event} week.`,
    ...
    forumNote: 'The forum runs for two days at Palazzo dei Congressi. The campaign runs all week in the shops across town.',
    ...
    rowTotal: 'Prize pool',
    valueNote: (date) => `Value as of ${date}: it can change at any time with the price of gold.`,
    valueLink: 'See the current rate',
  ...
  how: {
    ...
    networksTitle: 'Networks accepted for the campaign',
    networksNote: 'A payment on any other network does not qualify for the campaign.',
    networkOn: 'on',
  ...
  b2b: {
    ...
    perks: (pool) => [
      ...
      'You stay on the official list of participating shops — no confirmation, no listing',
    ...
    items: [
      {
        ...
        a: 'NAKA POS terminals take Bitcoin on the Lightning Network, USD₮ on Ethereum and Polygon, and Tether Gold on Ethereum. A payment on any other network does not qualify. Many merchants also accept LVGA, which however does not qualify for this campaign. Each merchant can enable one or more assets: availability is shown on the shop card in the map.',
      },
      {
  ...
  video: {
    ...
    merchantPrizes: {
      eyebrow: 'Every merchant prize',
      title: (pool) => `${pool} reserved for merchants`,
      subtitle:
        'The social prize is one of three. The other two need nothing published: one rewards how much you take in crypto, the other asks for a single transaction. They stack — one shop can win all three.',
      ...
      items: {
        ...
        'Estrazione Riservata Merchant': {
          title: 'Merchant-Only Draw',
          text: 'The easiest prize to reach: one crypto transaction taken during the week puts you in the draw. Every shop gets one ticket and one only — taking a hundred payments does not improve your odds.',
          action: 'Join and take at least one crypto payment. That is all.',
        },
      },
      note: (date) =>
        `Volumes and winners are settled once the campaign closes; the merchant-only draw takes place on ${date} using the same verifiable procedure as the customer draw. Prizes are paid in Tether Gold (XAUT) to the address provided by the winner.`,
    },
    merchantGuide: {
